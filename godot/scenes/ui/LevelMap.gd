...
func _ready():
	...
	# Set scroll position.
	set_scroll_ratio(1.0)


func get_scroll_ratio():
	var result
	if not (scroll_panel.rect_size.y - scroll_container.rect_size.y) == 0:
		result = scroll_container.get_v_scroll() / (scroll_panel.rect_size.y - scroll_container.rect_size.y)
	else:
		result = 0
	return result
...
func set_scroll_ratio(ratio):
	...
	scroll_container.set_v_scroll(ratio * (scroll_panel.rect_size.y - scroll_container.rect_size.y))
