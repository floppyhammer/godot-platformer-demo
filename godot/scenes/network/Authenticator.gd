...
# Authenticate a new session via CUSTOM USERNAME
func register_async(custom_id: String) -> int:
	var session : NakamaSession = yield(_client.authenticate_custom_async(custom_id), "completed")
