...
void fragment() {
	vec2 adjusted_uv = UV;
	adjusted_uv.y = adjusted_uv.y * aspect_ratio;
	
	vec2 noise2_uv = adjusted_uv * tile_factor2 + scroll_direction2 * TIME * time_scale2;
	float noise2 = texture(noise_texture2, noise2_uv).r;
	
	vec2 noise1_uv = adjusted_uv * tile_factor1 + scroll_direction1 * TIME * time_scale1;
